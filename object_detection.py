...
def upload_image():
    """
    Function to upload an image file using Google Colab's file upload tool.
    Returns:
        str: Path of the uploaded image.
    """
    uploaded = files.upload()  # Prompts user to upload an image file
    img_path = list(uploaded.keys())[0]  # Get the path of the first uploaded file
    return img_path
